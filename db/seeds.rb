...
def create_descriptor_instance(name, card)
    case name.downcase
    # Expansions
    when 'Base'.downcase
        Descriptor.create!({
    ...
    when 'Village'.downcase, 'Splitter'.downcase
        if(Descriptor.where(name: 'Splitter / Village').any?)
            return
        end
    ...
    when 'Big Money'.downcase
        ...
        raise "Invalid descriptor creation request. Params name: '#{name}' card: '#{card}'"
    end
end

def assign_descriptors(descriptors, card)
    descriptors.each do |kw|
        create_descriptor_instance(kw, card)
    end
end
...
assign_descriptors(%w(Base Action Trasher Trashing Terminal), card)

card = Card.create!(name: "Moat",
            image_url: base_url + "f/fe/Moat.jpg/200px-Moat.jpg",
            cost: 2)

assign_descriptors(%w(Base Action Reaction Blocker Terminal), card)
...
assign_descriptors(%w(Base Action Deck\ Discarder Virtual\ Coin Terminal), card)

card = Card.create!(name: "Village",
            image_url: base_url + "5/5a/Village.jpg/200px-Village.jpg",
            cost: 3)

assign_descriptors(%w(Base Action Splitter Village Non-Terminal Engine), card)

card = Card.create!(name: "Woodcutter",
            image_url: base_url + "d/d6/Woodcutter.jpg/200px-Woodcutter.jpg",
            cost: 3)

assign_descriptors(%w(Base Action +Buy Virtual\ Coin Engine Terminal), card)

card = Card.create!(name: "Workshop",
            image_url: base_url + "5/50/Workshop.jpg/200px-Workshop.jpg",
            cost: 3)

assign_descriptors(%w(Base Action Gainer Terminal), card)
...
assign_descriptors(%w(Base Action Attack Gainer Terminal), card)

card = Card.create!(name: "Feast",
            image_url: base_url + "9/9c/Feast.jpg/200px-Feast.jpg",
            cost: 4)

assign_descriptors(%w(Base Action One-Shot Terminal), card)

card = Card.create!(name: "Gardens",
            image_url: base_url + "8/8c/Gardens.jpg/200px-Gardens.jpg",
            cost: 4)

assign_descriptors(%w(Base Victory Rush alt-VP Terminal), card)

card = Card.create!(name: "Militia",
            image_url: base_url + "a/a0/Militia.jpg/200px-Militia.jpg",
            cost: 4)

assign_descriptors(%w(Base Action Attack Handsize\ Attack Virtual\ Coin Terminal), card)

card = Card.create!(name: "Moneylender",
            image_url: base_url + "7/70/Moneylender.jpg/200px-Moneylender.jpg",
            cost: 4)

assign_descriptors(%w(Base Action Trasher Trash-For-Benefit Trashing Terminal), card)
...
assign_descriptors(%w(Base Action Smithies Big\ Money Engine Terminal), card)

card = Card.create!(name: "Spy",
            image_url: base_url + "c/cb/c/cb/Spy.jpg/200px-Spy.jpg",
            cost: 4)

assign_descriptors(%w(Base Action Attack Deck\ Inspection\ Attack Cantrip), card)

card = Card.create!(name: "Thief",
            image_url: base_url + "f/f5/Thief.jpg/200px-Thief.jpg",
            cost: 4)

assign_descriptors(%w(Base Action Attack Trashing\ Attack Terminal), card)

card = Card.create!(name: "Throne Room",
            image_url: base_url + "d/d1/Throne_Room.jpg/200px-Throne_Room.jpg",
            cost: 4)

assign_descriptors(%w(Base Action Throne\ Room Duplicator Terminal), card)
