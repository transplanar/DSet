...
class Card < ActiveRecord::Base
  ...
  def self.search(queries_string, slot)
    ...
    # TODO: Restore save to slot functionality
  end
  ...
  private_class_method def self.format_multi_char_queries(arr)
    ...
    arr.each do |elem|
      result << (numeric?(elem) ? elem : format_string_query(elem))
    end
  ...
  private_class_method def self.format_string_query(query)
    ...
    formatted_query = REGEX_DELIMITER

    query.split('').each do |ch|
      formatted_query += (ch + REGEX_DELIMITER)
    end
  ...
  private_class_method def self.get_card_set(match_data)
    if(match_data.empty?)
      card_set = Card.all
      matched_columns = []
    else
      ...
      # TODO: Do something with matched terms
      # matched_terms = match_data.map{ |_, v| v[:terms] }.flatten.uniq
    end

    [card_set, matched_columns]
  end

  # TODO: Rename methods for readibility
  private_class_method def self.get_card_subset(query, match_data)
    new_match_data = {}

    card_set, matched_columns = get_card_set(match_data)
  ...
  private_class_method def self.numeric?(str)
    str.to_s.delete(REGEX_DELIMITER).match(/\A[+-]?\d+?(\.\d+)?\Z/) != nil
  end
  ...
  private_class_method def self.merge_match_data(match_data, new_match_data, card, column, query)
    ...
    new_columns = merge_result_hash(match_data, card.name, :columns, column)
    new_match_data[card.name][:columns] = new_columns

    new_terms = merge_result_hash(match_data, card.name, :terms, query)
    new_match_data[card.name][:terms] = new_terms
  end

  # FIXME: move to helper?
  private_class_method def self.merge_result_hash(hsh, key, sub_key, new_elem)
    return [new_elem] if hsh[key].nil? || hsh[key][sub_key].nil?
